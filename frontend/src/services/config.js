...
const assertPublishedConfigIsSafe = () => {
  if (isLocalHostname(window.location.hostname)) {
    return;
  }

  if (config.REACT_APP_BACKEND_URL) {
    const backendUrl = new URL(config.REACT_APP_BACKEND_URL);
    if (backendUrl.protocol !== "https:") {
      throw new Error("Published backend URL must use https");
    }

    if (isLocalHostname(backendUrl.hostname)) {
      throw new Error("Published backend URL cannot point to localhost");
    }

    return;
  }

  if ((config.BACKEND_PROTOCOL ?? "https") !== "https") {
    throw new Error("Published backend config must use https");
  }

  if (isLocalHostname(config.BACKEND_HOST)) {
    throw new Error("Published backend config cannot point to localhost");
  }
};

assertPublishedConfigIsSafe();
