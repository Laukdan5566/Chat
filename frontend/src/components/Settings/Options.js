...
export default function Options(props) {
  ...
  const [desktopDeviceSessionLimit, setDesktopDeviceSessionLimit] =
    useState("1");
  const [mobileDeviceSessionLimit, setMobileDeviceSessionLimit] =
    useState("1");
  ...
  useEffect(() => {
    ...
    if (Array.isArray(settings) && settings.length) {
      ...
      const legacyDeviceSessionLimit = settings.find(
        s => s.key === "deviceSessionLimit"
      );
      const desktopDeviceSessionLimit = settings.find(
        s => s.key === "desktopDeviceSessionLimit"
      );
      setDesktopDeviceSessionLimit(
        desktopDeviceSessionLimit?.value || legacyDeviceSessionLimit?.value || "1"
      );

      const mobileDeviceSessionLimit = settings.find(
        s => s.key === "mobileDeviceSessionLimit"
      );
      setMobileDeviceSessionLimit(
        mobileDeviceSessionLimit?.value || legacyDeviceSessionLimit?.value || "1"
      );

      const messageRetentionDays = settings.find(
        s => s.key === "messageRetentionDays"
      );
      setMessageRetentionDays(messageRetentionDays?.value || "365");
      ...
      const spyTicketVisibility = settings.find(
        s => s.key === "spyTicketVisibility"
      );
      setSpyTicketVisibility(spyTicketVisibility?.value || "admin");
  ...
  return (
    <>
      <Grid spacing={3} container>
        ...
        <Grid xs={12} sm={6} md={4} item>
          <FormControl className={classes.selectContainer}>
            <InputLabel id="spy-ticket-visibility-label">
              Espiar conversas
            </InputLabel>
            <Select
              labelId="spy-ticket-visibility-label"
              value={spyTicketVisibility}
              onChange={async e => {
                await handleSetting(
                  "spyTicketVisibility",
                  e.target.value,
                  setSpyTicketVisibility
                );
              }}
            >
              <MenuItem value={"disabled"}>Desativado</MenuItem>
              <MenuItem value={"admin"}>Somente administradores</MenuItem>
              <MenuItem value={"all"}>Todos os usuarios</MenuItem>
            </Select>
          </FormControl>
        </Grid>
          ...
          yes={() => (
            <>
              ...
              <Grid xs={12} sm={6} md={4} item>
                <FormControl className={classes.selectContainer}>
                  <TextField
                    id="desktop-device-session-limit-field"
                    label="Limite por usuario - Computador"
                    variant="standard"
                    name="desktopDeviceSessionLimit"
                    type="number"
                    value={desktopDeviceSessionLimit}
                    helperText="0 desativa o limite"
                    onChange={e => {
                      setDesktopDeviceSessionLimit(e.target.value);
                    }}
                    onBlur={async _ => {
                      await handleSetting(
                        "desktopDeviceSessionLimit",
                        desktopDeviceSessionLimit
                      );
                    }}
                  />
              ...
              <Grid xs={12} sm={6} md={4} item>
                <FormControl className={classes.selectContainer}>
                  <TextField
                    id="mobile-device-session-limit-field"
                    label="Limite por usuario - Mobile"
                    variant="standard"
                    name="mobileDeviceSessionLimit"
                    type="number"
                    value={mobileDeviceSessionLimit}
                    helperText="0 desativa o limite"
                    onChange={e => {
                      setMobileDeviceSessionLimit(e.target.value);
                    }}
                    onBlur={async _ => {
                      await handleSetting(
                        "mobileDeviceSessionLimit",
                        mobileDeviceSessionLimit
                      );
                    }}
                  />
              ...
              <Grid xs={12} sm={6} md={4} item>
                <FormControl className={classes.selectContainer}>
                  <TextField
                    id="message-retention-days-field"
                    label="Retencao de mensagens (dias)"
                    variant="standard"
                    name="messageRetentionDays"
                    type="number"
                    value={messageRetentionDays}
                    helperText="0 desativa a limpeza automatica"
                    onChange={e => {
                      setMessageRetentionDays(e.target.value);
                    }}
                    onBlur={async _ => {
                      await handleSetting(
