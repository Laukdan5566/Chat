...
export function CompanyForm(props) {
  ...
  const [salesRoutingLoading, setSalesRoutingLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    api
      .get(`/sales-routing/company/${initialValue.id}`)
      .then(({ data }) => {
        if (active) setSalesRoutingEnabled(Boolean(data?.enabled));
      })
      .catch(() => {
  ...
  const handleSalesRoutingChange = async event => {
    if (!initialValue?.id) return;

    setSalesRoutingLoading(true);
    try {
      const { data } = await api.put(
        `/sales-routing/company/${initialValue.id}`,
        { enabled: event.target.checked }
      );
      setSalesRoutingEnabled(Boolean(data.enabled));
      toast.success(
        data.enabled
          ? "Roteamento comercial ativado para esta empresa."
          : "Roteamento comercial desativado para esta empresa."
      );
    } catch (error) {
      toast.error("N\u00e3o foi poss\u00edvel atualizar o roteamento comercial.");
    } finally {
      setSalesRoutingLoading(false);
    }
  };
            ...
            <Grid spacing={2} justifyContent="flex-end" container>
              ...
              <Grid xs={12} sm={6} md={2} item>
                <FormControl margin="dense" variant="outlined" fullWidth>
                  ...
                  <Field
                    ...
                    id="zammad-selection"
                    label="Zammad"
                    labelId="zammad-selection-label"
                    name="zammadEnabled"
                    margin="dense"
                  >
              ...
              <Grid xs={12} sm={6} md={4} item>
                <Field
                  ...
                  className={classes.fullWidth}
                  margin="dense"
                  placeholder="https://zammad.fpinformatica.com.br"
                />
              </Grid>
              <Grid xs={12} sm={6} md={3} item>
                <Field
                  as={TextField}
                  label="Token do Zammad"
                  name="zammadToken"
                  type="password"
                  variant="outlined"
                  className={classes.fullWidth}
              ...
              {record.id !== undefined && (
                <Grid xs={12} item>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={salesRoutingEnabled}
                        color="primary"
                        disabled={salesRoutingLoading}
                        onChange={handleSalesRoutingChange}
                      />
                    }
                    label="Ativar roteamento comercial nesta empresa"
                  />
                </Grid>
              )}
              <Grid xs={12} item>
...
export function CompaniesManagerGrid(props) {
  ...
  const renderPlanChannels = row => {
    const channels = ["WhatsApp"];

    if (row.plan?.facebookEnabled) {
      channels.push("Facebook");
    }

    if (row.plan?.instagramEnabled) {
  ...
  const renderZammadStatus = row => {
    if (
      ...
      const setting = row.settings.find(s => s.key === "zammadEnabled");
      if (setting) {
        ...
        <TableBody>
          {records.map((row, key) => (
            <TableRow className={rowClass(row)} key={key}>
              ...
              <TableCell align="left" style={{ color: "unset" }}>
                {renderPlanChannels(row)}
              </TableCell>
              <TableCell align="left" style={{ color: "unset" }}>
                {renderCampaignsStatus(row)}
              </TableCell>
              <TableCell align="left" style={{ color: "unset" }}>
                {renderZammadStatus(row)}
              </TableCell>
              <TableCell align="left" style={{ color: "unset" }}>
                {renderStatus(row)}
              </TableCell>
...
export default function CompaniesManager() {
  ...
  const handleSelect = data => {
    ...
    const zammadSetting = data.settings.find(s => s.key === "zammadEnabled");
    if (zammadSetting) {
      zammadEnabled =
        zammadSetting.value === "true" || zammadSetting.value === "enabled";
    }
    ...
    setRecord(prev => ({
      ...
      zammadPriority: getSettingValue(
        data.settings,
        "zammadPriority",
        "2 normal"
      ),
