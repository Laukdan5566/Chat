...
const TicketsListCustom = props => {
  ...
  useEffect(() => {
    ...
    const onCompanyTicket = data => {
      ...
      if (
        ["update", "participantAdded", "participantRemoved"].includes(
          data.action
        ) &&
      ...
      if (
        ["update", "participantRemoved"].includes(data.action) &&
    ...
    const onCompanyAppMessage = data => {
      ...
      const queueIds = queues.map(q => q.id);
      const isParticipant = isTicketParticipant(data.ticket);
      if (
        profile === "user" &&
        !isParticipant &&
