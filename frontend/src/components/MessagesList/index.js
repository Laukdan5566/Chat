...
const useStyles = makeStyles(theme => ({
  ...
  internalNoteContainer: {
    ...
    backgroundColor:
      theme.mode === "light" ? "rgba(255, 243, 205, 0.96)" : "#4a3b12",
    color: theme.mode === "light" ? "#4a3b12" : "#fff4c2",
    boxShadow:
      theme.mode === "light" ? "0 1px 1px #b3b3b3" : "0 1px 1px #000000",
    whiteSpace: "pre-wrap",
...
const reducer = (state, action) => {
  ...
  if (action.type === "UPDATE_MESSAGE_ACKS") {
    let changed = false;
    ...
    action.payload.forEach(messageToUpdate => {
      ...
      if (
        messageIndex !== -1 &&
        Number(state[messageIndex].ack) !== Number(messageToUpdate.ack)
      ) {
        state[messageIndex] = {
          ...state[messageIndex],
          ack: messageToUpdate.ack,
          updatedAt: messageToUpdate.updatedAt
        };
        changed = true;
      }
    });
...
const MessagesList = ({ ticket, ticketId, isGroup, markAsRead, readOnly }) => {
  ...
  const [contactHistoryMessages, setContactHistoryMessages] = useState([]);
  const [contactHistoryPageNumber, setContactHistoryPageNumber] = useState(1);
  const [contactHistoryHasMore, setContactHistoryHasMore] = useState(false);
  ...
  async function refreshPendingAcks() {
    try {
      const { data } = await api.get("/messages/" + ticketId, {
        params: { pageNumber: 1, markAsRead: false }
      });

      if (currentTicketId.current === ticketId) {
        dispatch({ type: "UPDATE_MESSAGE_ACKS", payload: data.messages });
      }
    } catch (_) {
      // A proxima atualizacao por socket ou novo ciclo corrige o estado.
    }
  }

  useEffect(async () => {
    dispatch({ type: "RESET" });
    setContactHistoryMessages([]);
    setContactHistoryPageNumber(1);
    setContactHistoryHasMore(false);
    setContactHistoryLoading(false);
    setContactHistoryLoaded(false);
  ...
  useEffect(() => {
    ...
    if (!ticketId || loading || !hasPendingOwnAck) {
      return undefined;
    }

    const ackRefreshTimeout = setTimeout(refreshPendingAcks, 2500);

    return () => clearTimeout(ackRefreshTimeout);
  }, [messagesList, ticketId, loading]);
  ...
  const loadContactHistory = async () => {
    ...
    if (!contactId || contactHistoryLoading) {
      return;
    }

    const nextPage = contactHistoryLoaded ? contactHistoryPageNumber + 1 : 1;
    ...
    try {
      ...
      const historyMessages = data.messages.map(message => ({
        ...message,
        isContactHistory: true
      }));

      setContactHistoryMessages(prevMessages => [
        ...historyMessages,
        ...prevMessages
      ]);
      setContactHistoryPageNumber(nextPage);
      setContactHistoryHasMore(data.hasMore);
  ...
  const handleScroll = e => {
    ...
    if (scrollTop < 50) {
      ...
      if (!contactHistoryLoaded && !contactHistoryLoading) {
        loadContactHistory();
      }
    }
  };
  ...
  const renderMessageAck = message => {
    if (message.channel === "webchat") {
      return Number(message.ack) >= 3 ? (
        <DoneAll fontSize="small" className={classes.ackIcons} />
      ) : (
  ...
  const renderDailyTimestamps = (message, index, displayMessages) => {
    if (index === 0) {
      return (
        <span
        ...
        >
          <div className={classes.dailyTimestampText}>
            {format(parseISO(displayMessages[index].createdAt), "dd/MM/yyyy")}
          </div>
        </span>
      );
    }
    if (index < displayMessages.length) {
      let messageDay = parseISO(displayMessages[index].createdAt);
      let previousMessageDay = parseISO(displayMessages[index - 1].createdAt);
  ...
  const renderMessageDivider = (message, index, displayMessages) => {
    if (index < displayMessages.length && index > 0) {
      let messageUser = displayMessages[index].fromMe;
      let previousMessageUser = displayMessages[index - 1].fromMe;
  ...
  const renderMessages = () => {
    ...
    const displayMessages = [...contactHistoryMessages, ...messagesList];
    const viewMessagesList = displayMessages.map((message, index) => {
      if (message.mediaType === "reactionMessage") {
        return;
      }

      const data = JSON.parse(message.dataJson);
      if (message.mediaType === "internalNote") {
        return (
          <React.Fragment key={message.id}>
            {renderDailyTimestamps(message, index, displayMessages)}
            <div id={message.id} className={classes.internalNoteContainer}>
              <span className={classes.internalNoteTitle}>
                Anotacao interna
              </span>
              <WhatsMarked>{message.body}</WhatsMarked>
              <span className={classes.internalNoteTime}>
      ...
      if (!message.fromMe) {
        const messageFragment = (
          <React.Fragment key={message.id}>
            {renderDailyTimestamps(message, index, displayMessages)}
            {renderMessageDivider(message, index, displayMessages)}
            <div
              id={message.id}
              className={[
    ...
    const renderedMessages = [...viewMessagesList];
    if (contactHistoryMessages.length > 0 && messagesList.length > 0) {
      renderedMessages.splice(
        contactHistoryMessages.length,
        0,
        <div
          className={classes.contactHistoryDivider}
          key="current-ticket-divider"
        >
          Conversa atual
        </div>
      );
    ...
    return (
      <>
        {viewMessagesList.length > 0 && contactHistoryMessages.length > 0 && (
          <div className={classes.contactHistoryDivider}>
            Historico anterior deste numero
          </div>
        )}
        {renderedMessages}
        <div
  ...
  return (
    <div className={classes.messagesListWrapper}>
      ...
      <div
        ...
        ref={scrollRef}
      >
        {!hasMore && (ticket?.contactId || ticket?.contact?.id) && (
          <div className={classes.contactHistoryLoader}>
            <Typography variant="caption">
              {contactHistoryLoaded && contactHistoryMessages.length === 0
                ? "Nao ha historico anterior para este numero."
                : contactHistoryLoaded
                ? "Ainda quer ver mais historico deste numero?"
                : "Chegou ao inicio deste ticket."}
            </Typography>
            {(contactHistoryHasMore || !contactHistoryLoaded) && (
              <Button
                size="small"
                variant="outlined"
                color="primary"
              ...
              >
                {contactHistoryLoading
                  ? "Carregando..."
                  : "Carregar mensagens anteriores deste numero"}
              </Button>
            )}
          </div>
        )}
        {messagesList.length > 0 || contactHistoryMessages.length > 0
          ? renderMessages()
          : []}
