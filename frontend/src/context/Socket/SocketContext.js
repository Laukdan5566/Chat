  ...
  GetSocket: function (_discardCompanyId = null) {
    ...
    if (companyId !== this.currentCompanyId || userId !== this.currentUserId) {
      ...
      this.currentSocket.onAny((event, ...args) => {
        ...
        if (event === `company-${companyId}-settings`) {
          const payload = args[0];
          const setting = payload?.setting;
          if (payload?.action === "update" && setting?.key) {
            sessionStorage.setItem(setting.key, JSON.stringify(setting.value));
            sessionStorage.setItem(`${setting.key}_timestamp`, Date.now());
            window.dispatchEvent(
              new CustomEvent("ticketz-settings-updated", {
                detail: setting
              })
            );
          }
