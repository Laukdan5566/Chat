...
import { hasUserPermission } from "../../helpers/userPermissions.js";
...
const Dashboard = () => {
  ...
  function renderFilters() {
    return (
      <Grid item xs={12} className={classes.filtersRow}>
  ...
  if (!hasUserPermission(currentUser, "dashboard:view")) {
    return <div></div>;
  }

  const disconnectedConnections = Math.max(
    platformSummary.totalConnections - platformSummary.connectedConnections,
    0
  );
  const disconnectedCompanies = Math.max(
    platformSummary.totalCompanies - platformSummary.connectedCompanies,
    0
  );
  const otherMobileSessions = Math.max(
    platformSummary.activeMobileSessions - platformSummary.activeAppSessions,
    0
  );
          ...
          {currentUser.super && (
            <>
              <InfoRingCard
                ...
                value={`${platformSummary.connectedConnections}/${platformSummary.totalConnections}`}
                graph={
                  <SmallPie
                    chartData={[
                      {
                        name: "Conectadas",
                        value: platformSummary.connectedConnections,
                        color: "#00ff00"
                      },
                      {
                        name: "Desconectadas",
                        value: disconnectedConnections,
                        color: "#ff0000"
                      }
                    ]}
                  />
              ...
              <InfoRingCard
                ...
                value={`${platformSummary.connectedCompanies}/${platformSummary.totalCompanies}`}
                graph={
                  <SmallPie
                    chartData={[
                      {
                        name: "Conectadas",
                        value: platformSummary.connectedCompanies,
                        color: "#00ff00"
                      },
                      {
                        name: "Sem conexão",
                        value: disconnectedCompanies,
                        color: "#ff0000"
                      }
                    ]}
                  />
              ...
              <InfoRingCard
                title="Acessos via app"
                value={platformSummary.activeAppSessions || 0}
                graph={
                  <SmallPie
                    chartData={[
                      {
                      ...
                      {
                        name: "Mobile web",
                        value: otherMobileSessions,
                        color: "#1976d2"
                      },
                      {
                        name: "Desktop",
                        value: platformSummary.activeDesktopSessions || 0,
                        color: "#6a35a8"
                      }
                    ]}
                  />
