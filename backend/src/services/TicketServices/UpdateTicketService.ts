...
import { getSalesRoutingConsultantQueueForUser } from "../SalesRoutingServices/SalesRoutingService";
...
const sendFormattedMessage = async (
  ...
  try {
    ...
    logger.warn(
      ...
      "Failed to send automatic ticket status message"
    );
  }
};
...
const UpdateTicketService = async ({
  ...
  try {
    ...
    const userBelongsToQueue = (id?: number | null) =>
      !id || userQueueIds.includes(Number(id));
    const isAssigningToUser = userId !== undefined && userId !== null;

    if (isAssigningToUser && queueId === null) {
      queueId = ticket.queueId;
    }

    // A plain accept from the commercial reception queue belongs in the
    // consultant's own queue. Explicit queue choices still win, and this only
    // runs for companies that have commercial routing enabled.
    const isAcceptingForSelf =
      ticket.status === "pending" &&
      status === "open" &&
      Number(userId) === Number(user?.id) &&
      queueId === undefined &&
      userBelongsToQueue(ticket.queueId);

    if (isAcceptingForSelf) {
      const consultantQueueId = await getSalesRoutingConsultantQueueForUser({
        companyId: ticket.companyId,
        currentQueueId: ticket.queueId,
        userQueueIds
      });

      if (consultantQueueId) {
        queueId = consultantQueueId;
      }
    }

    const destinationQueueId =
      queueId !== undefined && queueId !== null ? queueId : ticket.queueId;
    ...
    if (userId !== undefined && userId !== null && userId !== user?.id) {
      targetUser = await User.findByPk(userId, {
        include: [{ model: Queue, attributes: ["id"] }]
      });

      if (!targetUser || targetUser.companyId !== ticket.companyId) {
        throw new AppError("ERR_FORBIDDEN", 403);
      }

      const targetQueueIds =
        targetUser.queues?.map(queue => Number(queue.id)) || [];

      if (
        !destinationQueueId ||
        !targetQueueIds.includes(Number(destinationQueueId))
      ) {
        throw new AppError("ERR_FORBIDDEN", 403);
      }
    }
    ...
    if (user && !isAdmin) {
      const isOwnedTicket = Number(ticket.userId) === Number(user.id);
      const isDirectTransferToAnotherUser =
        userId !== undefined &&
        userId !== null &&
        Number(userId) !== Number(user.id);
      ...
      if (
        queueId !== undefined &&
        queueId !== null &&
        !userBelongsToQueue(queueId) &&
        !(isOwnedTicket && isDirectTransferToAnotherUser)
      ) {
        throw new AppError("ERR_FORBIDDEN", 403);
      }

      if (ticket.status === "pending") {
        const canHandlePendingTicket =
          userBelongsToQueue(ticket.queueId) ||
          (queueId !== undefined && userBelongsToQueue(queueId));

        if (!canHandlePendingTicket) {
          throw new AppError("ERR_FORBIDDEN", 403);
        }
    ...
    if (!oldQueueId && userId && oldStatus === "pending" && status === "open") {
      ...
      if (
        ...
        (!queueId || !userBelongsToQueue(queueId))
      ) {
    ...
    if (status !== undefined && ["open"].indexOf(status) > -1) {
      ...
      // Remove the card only from its previous list. Broadcasting this through
      // mainchannel removed it from the open list too, leaving the UI empty
      // until the next full fetch.
      if (oldStatus && oldStatus !== status) {
        let previousList = io.to(`company-${companyId}-${oldStatus}`);

        if (oldQueueId) {
          previousList = previousList.to(
            `queue-${oldQueueId}-${oldStatus}`
          );
        }

        previousList.emit(`company-${companyId}-ticket`, {
          action: "removeFromList",
          ticketId: ticket.id
        });
      }
      ...
      // Publish the accepted ticket immediately. Automatic status messages can
      // take longer, but the operator must see the conversation at once.
      websocketUpdateTicket(ticket, [`user-${oldUserId}`]);
      ticketUpdateEmitted = true;
    }
