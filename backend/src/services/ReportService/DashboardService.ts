...
export async function platformStatusSummary(): Promise<PlatformStatusSummary> {
  const [
    totalConnections,
...
export async function statusSummaryService(companyId: number, isSuper = false) {
  return {
    ticketsStatusSummary: await ticketsStatusSummary(companyId),
    usersStatusSummary: await usersStatusSummary(companyId),
    platformSummary: isSuper ? await platformStatusSummary() : null
  };
}
