        ...
        wsocket.ev.on(
          ...
          async ({ connection, lastDisconnect, qr, reachoutTimeLock }) => {
            ...
            if (connection === "close") {
              ...
              const payload = disconnectError?.output?.payload as
                | { error?: string; message?: string }
                | undefined;
              const disconnectMessage =
                payload?.message || disconnectError?.message || "";
              const disconnectPayloadError = payload?.error || "";
              const isConflictOrUnauthorized =
                statusCode === 401 ||
                statusCode === 403 ||
                disconnectPayloadError === "Unauthorized" ||
                disconnectMessage.toLowerCase().includes("conflict");

              if (isConflictOrUnauthorized) {
                // disconnected from whatsapp
                await removeWbot(id, statusCode === 403);
                await whatsapp.update({
                  status: "DISCONNECTED",
                  session: statusCode === 403 ? "" : whatsapp.session,
                  qrcode: ""
                });
                if (statusCode === 403) {
            ...
            if (connection === "open") {
              wsocket
                ...
                .catch(error => {
                  logger.warn(
                    { error },
                    `Error fetching reachout timelock for session ${name}`
                  );
                });
              ...
              wsocket
                ...
                .catch(error => {
                  logger.warn(
                    { error },
                    `Error fetching new chat message cap for session ${name}`
                  );
                });
