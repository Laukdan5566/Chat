...
import { dispatchWaitingSalesRoutingTicket } from "../services/SalesRoutingServices/SalesRoutingService";
...
const userHasTicketQueue = (user: User, ticket: Ticket): boolean => {
...
const notifySalesRoutingDispatch = (ticket: Ticket) => {
...
export const initIO = (httpServer: Server): SocketIO => {
  ...
  io.on("connection", async socket => {
    ...
    const wasAlreadyOnline =
      (await UserSocketSession.count({ where: { userId, active: true } })) > 0;

    await UserSocketSession.create({
      id: socket.id,
      userId,
      active: true,
      deviceType,
    ...
    logger.debug(`started ${deviceType} session ${socket.id} for user ${userId}`);
    notifyOnlineChange(user.companyId, user.id, true);

    if (!wasAlreadyOnline) {
      try {
        const dispatchedTicket = await dispatchWaitingSalesRoutingTicket({
          companyId: user.companyId,
          userId: user.id
        });
        if (dispatchedTicket) {
          notifySalesRoutingDispatch(dispatchedTicket);
        }
      } catch (error) {
        logger.error(
          { error, companyId: user.companyId, userId: user.id },
          "Failed to dispatch a waiting sales-routing ticket"
        );
      }
    }
    ...
    socket.on("joinChatBox", async (ticketId: string) => {
      ...
      Ticket.findByPk(ticketId).then(
        async ticket => {
          const spyTicketVisibility = await GetCompanySetting(
            user.companyId,
            "spyTicketVisibility",
            "admin"
          );
          const canSpyTicket =
            ...
            ticket &&
            userHasTicketQueue(user, ticket);
          const canViewAsParticipant =
            ticket &&
            hasPermission(user, "ticket-participants:view") &&
            (await TicketParticipant.count({
              where: {
                ticketId: ticket.id,
                userId: user.id
              }
