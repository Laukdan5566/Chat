  ...
  try {
    ...
    if (schedule.saveMessage) {
      handleMessage(
        ...
        schedule.companyId,
        scheduleRecord?.queueId || schedule.queueId || undefined
      );
    }
...
async function handleMessageRetention() {
  const retentionDays = Number.parseInt(
    await CheckSettings("messageRetentionDays", "365"),
    10
  );
  ...
  const replacements = { cutoff };

  await sequelize.query(
    `
      DELETE FROM "OldMessages" old
      USING "Messages" msg
      WHERE old."messageId" = msg."id"
        AND msg."createdAt" < :cutoff
    `,
    { replacements }
  );
  ...
  await sequelize.query(
    `
      ...
      WHERE old."ticketId" = ticket."id"
        AND old."createdAt" < :cutoff
    `,
    { replacements }
  );
  ...
  await sequelize.query(
    `
      UPDATE "Messages" msg
      SET "quotedMsgId" = NULL
      FROM "Messages" quoted
      WHERE msg."quotedMsgId" = quoted."id"
        AND quoted."createdAt" < :cutoff
    `,
    { replacements }
  );

  const removed = await MessageModel.destroy({
    where: {
      createdAt: {
        [Op.lt]: cutoff
      }
  ...
  if (removed) {
    logger.info(
      ...
      "message retention removed old messages"
    );
  }
}
...
const messageRetentionJob = new CronJob("0 20 3 * * *", async () => {
  try {
    await handleMessageRetention();
  } catch (e) {
    logger.error(
      { message: e?.message },
      "messageRetentionJob: error received"
    );
  }
});

messageRetentionJob.start();
